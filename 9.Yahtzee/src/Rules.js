...
class FullHouse extends Rule{
  ...
  evalRoll = (dice) => {
    if (this.freq(dice).length === 2) {
      if (this.freq(dice)[0] === 2 || this.freq(dice)[1] === 3) {
        return this.score;
      }
    } else {
      return 0;
...
class SmallStraight extends Rule{
  // TODO
  evalRoll=(dice)=>{
      const ToBeChecked=this.freq(dice)
      let result_case1 =ToBeChecked.includes(1,2,3,4);
      
      let result_case2 =ToBeChecked.includes(2,3,4,5);

      let result_case3 =ToBeChecked.includes(3,4,5,6);

      return (result_case1 || result_case2) || result_case3 ? this.score :  0;
  }
}
